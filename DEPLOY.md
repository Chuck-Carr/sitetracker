# SiteStratus — Production Deployment Guide

Step-by-step instructions for deploying SiteStratus on a fresh Ubuntu server.
...
- DNS A record for `sitestratus.com` pointing at your server's public IP
  *(and an A record for `www.sitestratus.com` if desired)*
...
```bash
cd ~
git clone https://github.com/YOUR_USERNAME/YOUR_REPO.git sitestratus
cd sitestratus
```
...
rsync -avz \
  ...
  user@YOUR_SERVER_IP:~/sitestratus/
```
...
```
NEXT_PUBLIC_APP_URL=https://sitestratus.com
```
...
```
DATABASE_URL=postgresql://sitestratus:YOUR_PASSWORD@postgres:5432/sitestratus
```
...
```bash
docker compose -p sitestratus -f docker-compose.prod.yml up -d postgres
```

Wait about 10 seconds, then confirm it is healthy:

```bash
docker compose -p sitestratus -f docker-compose.prod.yml ps
```
...
```bash
docker compose -p sitestratus -f docker-compose.prod.yml build app
```
...
```bash
docker compose -p sitestratus -f docker-compose.prod.yml up -d
```
...
```bash
# All services
docker compose -p sitestratus -f docker-compose.prod.yml logs -f

# Caddy only (watch for certificate issuance)
docker compose -p sitestratus -f docker-compose.prod.yml logs -f caddy

# App only
docker compose -p sitestratus -f docker-compose.prod.yml logs -f app
```

Caddy will log something like this when the certificate is ready:

```
certificate obtained successfully  {"domain": "sitestratus.com"}
```
...
1. Open `https://sitestratus.com` in a browser.
...
- DNS may not have propagated yet. Test with: `dig +short sitestratus.com`
- Check ports: `sudo ufw status` and confirm router forwarding.
- Check Caddy: `docker compose -p sitestratus -f docker-compose.prod.yml logs caddy`
...
- Restart: `docker compose -p sitestratus -f docker-compose.prod.yml restart app`
...
docker compose -p sitestratus -f docker-compose.prod.yml build app
docker compose -p sitestratus -f docker-compose.prod.yml up -d app
```
...
docker compose -p sitestratus -f docker-compose.prod.yml exec postgres \
...
docker compose -p sitestratus -f docker-compose.prod.yml exec -T postgres \
...
```bash
sudo tee /usr/local/bin/sitestratus-backup > /dev/null << 'EOF'
#!/bin/bash
set -euo pipefail
APP_DIR=/home/$(logname)/sitestratus
BACKUP_DIR=/var/backups/sitestratus
mkdir -p "$BACKUP_DIR"
cd "$APP_DIR"
set -a; source .env; set +a
docker compose -p sitestratus -f docker-compose.prod.yml exec -T postgres \
...
EOF
sudo chmod +x /usr/local/bin/sitestratus-backup
```

Schedule it to run daily at 3 AM:

```bash
(crontab -l 2>/dev/null; echo "0 3 * * * /usr/local/bin/sitestratus-backup >> /var/log/sitestratus-backup.log 2>&1") | crontab -
```
...
alias dc='docker compose -p sitestratus -f ~/sitestratus/docker-compose.prod.yml'
...
```bash
docker compose -p sitestratus -f docker-compose.prod.yml \
  exec postgres psql -U sitestratus sitestratus
```
